"""Filters for nfl data API endpoints."""

import django_filters
from django.db.models import Q, QuerySet

from apps.nfl.models import Event, Team
...
class EventFilter(django_filters.FilterSet):
    ...
    team = django_filters.CharFilter(
        method="team_filter",
        help_text="Filter by team nfl ID or abbreviation",
    )
    ...
    def team_filter(
        ...
        """Filter events by team (nfl ID or abbreviation)."""
        if not value:
            return queryset
        return queryset.filter(
            Q(competitors__team__nfl_id=value)
