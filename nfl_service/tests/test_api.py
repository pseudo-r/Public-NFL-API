...
class TestTeamEndpoints:
    ...
    def test_get_team_by_nfl_id(self, api_client: APIClient, team: Team):
        """Test getting team by nfl ID."""
        response = api_client.get("/api/v1/teams/nfl/1/")

        assert response.status_code == status.HTTP_200_OK
        assert response.json()["nfl_id"] == "1"

    def test_get_team_by_nfl_id_not_found(self, api_client: APIClient, league: League):
        """Test getting non-existent team by nfl ID."""
        response = api_client.get("/api/v1/teams/nfl/999/")
...
class TestEventEndpoints:
    ...
    def test_list_events(
        ...
        assert response.json()["count"] == 1
        result = response.json()["results"][0]
        assert result["nfl_id"] == "401584666"
    ...
    def test_get_event_by_nfl_id(self, api_client: APIClient, event: Event):
        """Test getting event by nfl ID."""
        response = api_client.get("/api/v1/events/nfl/401584666/")

        assert response.status_code == status.HTTP_200_OK
        assert response.json()["nfl_id"] == "401584666"

    def test_get_event_by_nfl_id_not_found(self, api_client: APIClient, league: League):
        """Test getting non-existent event by nfl ID."""
        response = api_client.get("/api/v1/events/nfl/999999/")
...
class TestIngestEndpoints:
    ...
    def test_ingest_teams_success(
        ...
        with patch("apps.ingest.services.get_nfl_client") as mock_get_client:
            mock_client = MagicMock()
            mock_client.get_teams.return_value = nflResponse(
    ...
    def test_ingest_scoreboard_success(
        ...
        with patch("apps.ingest.services.get_nfl_client") as mock_get_client:
            mock_client = MagicMock()
            mock_client.get_scoreboard.return_value = nflResponse(
