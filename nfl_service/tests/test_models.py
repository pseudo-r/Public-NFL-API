"""Tests for nfl models."""

from datetime import UTC, datetime

import pytest

from apps.nfl.models import Athlete, Competitor, Event, League, Sport, Team, Venue
...
class TestTeamModel:
    ...
    def test_team_unique_constraint(self, team: Team, league: League):
        """Test Team unique constraint on league+nfl_id."""
        with pytest.raises(Exception):  # IntegrityError
            Team.objects.create(
                league=league,
                nfl_id="1",  # Same as team fixture
                abbreviation="DUP",
...
class TestEventModel:
    ...
    def test_event_unique_constraint(self, event: Event, league: League):
        """Test Event unique constraint on league+nfl_id."""
        with pytest.raises(Exception):  # IntegrityError
            Event.objects.create(
                league=league,
                nfl_id="401584666",  # Same as event fixture
                date=datetime.now(UTC),
