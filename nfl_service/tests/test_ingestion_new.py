...
from apps.nfl.models import Injury, League, NewsArticle, Sport, Team, Transaction
...
def _make_nfl_response(data: dict) -> MagicMock:
    resp = MagicMock()
    resp.data = data
    return resp
...
class TestNewsIngestionService(TestCase):
    ...
    def test_ingest_creates_new_articles(self) -> None:
        ...
        self.client_mock.get_news.return_value = _make_nfl_response({
            "articles": [
                {
    ...
    def test_ingest_updates_existing_article(self) -> None:
        """Ingesting the same nfl_id twice should update, not create."""
        self.client_mock.get_news.return_value = _make_nfl_response({
            "articles": [
                {
                    "dataSourceIdentifier": "art-001",
        ...
        NewsArticle.objects.create(nfl_id="art-001", headline="Old headline")

        result = self.service.ingest_news("basketball", "nba")

        assert result.created == 0
        assert result.updated == 1
        assert NewsArticle.objects.get(nfl_id="art-001").headline == "Updated headline"

    def test_ingest_empty_response_returns_zero(self) -> None:
        self.client_mock.get_news.return_value = _make_nfl_response({"articles": []})
        result = self.service.ingest_news("basketball", "nba")
        assert result.created == 0
        assert result.updated == 0
        assert result.errors == 0

    def test_ingest_skips_items_without_nfl_id(self) -> None:
        self.client_mock.get_news.return_value = _make_nfl_response({
...
class TestInjuryIngestionService(TestCase):
    ...
    def test_ingest_creates_injury_records(self) -> None:
        self.client_mock.get_league_injuries.return_value = _make_nfl_response({
            "items": [
                {
    ...
    def test_ingest_empty_response(self) -> None:
        self.client_mock.get_league_injuries.return_value = _make_nfl_response({"items": []})
        result = self.service.ingest_injuries("football", "nfl")
        assert result.created == 0
...
class TestTransactionIngestionService(TestCase):
    ...
    def test_ingest_creates_transactions(self) -> None:
        self.client_mock.get_league_transactions.return_value = _make_nfl_response({
            "items": [
                {
                    "id": "txn-101",
        ...
        assert result.created == 1
        txn = Transaction.objects.get(nfl_id="txn-101")
        assert txn.type == "Signing"
        assert txn.date == date(2024, 7, 15)

    def test_ingest_updates_existing_transaction(self) -> None:
        Transaction.objects.create(league=self.league, nfl_id="txn-101", description="Old")
        ...
        assert result.updated == 1
        assert Transaction.objects.get(nfl_id="txn-101").description == "Updated description"
...
class TestNewsArticleAPI:
    ...
    def test_list_news_filter_by_league(self) -> None:
        ...
        NewsArticle.objects.create(nfl_id="a1", headline="NBA news", league=league)
        NewsArticle.objects.create(nfl_id="a2", headline="NFL news", league=league2)
